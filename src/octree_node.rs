...
// External Library
use cgmath::Point3;
use collision::{Frustum, Aabb, Aabb3};

use {Surround, MinVolume, OctreeItem};
...
impl<'a, I> OctreeNode<I> where I: OctreeItem + Clone + Debug
{
    ...
    pub fn insert_safe(&mut self, item: I, min_side: f32) -> bool {
                ...
                match node_type {
                    NodeType::Leaf(leaf_item) => {
                        if !leaf_item.is_equal(&item) {
                            if !node.aabb.is_min(min_side) {
                                node.subdivide();
                                node.insert_safe(leaf_item, min_side);
                                stack.push(node);
                            } else {
